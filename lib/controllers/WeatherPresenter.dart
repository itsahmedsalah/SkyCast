...
class WeatherPresenter extends GetxController {
  ...
  WeatherPresenter({required this.weatherUseCase, required this.locationService}) {
  ...
  Future<void> getWeather() async {
    try {
      ...
      String? cityName = await locationService.getCityFromCoordinates(position.latitude, position.longitude);
      String? countryName = await locationService.getCountryFromCoordinates(position.latitude, position.longitude);
      
      if (cityName != null) city.value = cityName;
      if (countryName != null) country.value = countryName;

      // Fetch weather with dynamic coordinates
      weather(await weatherUseCase.getCurrentWeather(position.latitude, position.longitude));
      hourlyWeather(await weatherUseCase.getHourlyWeather(position.latitude, position.longitude));
      dailyWeather(await weatherUseCase.getDailyWeather(position.latitude, position.longitude));
    } catch (e) {
